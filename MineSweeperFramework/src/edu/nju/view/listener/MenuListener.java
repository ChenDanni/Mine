...
public class MenuListener implements ActionListener{
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == ui.getMenuItem("start")) {//生成游戏，默认生成简单游戏
			...
			CustomDialog dialog = new CustomDialog(ui.getMainFrame());
			dialog.show();
			settingController.setCustomizedGameLevel(dialog.getHeight(), dialog.getWidth(), dialog.getMineNumber());
			menuController.startGame();
		} else if (e.getSource() == ui.getMenuItem("exit")) {
			System.exit(0);
		} else if (e.getSource() == ui.getMenuItem("record")) {//统计胜率信息
			RecordDialog diagol = new RecordDialog(ui.getMainFrame());
			diagol.show();
